...
# Initialize LLM & Embeddings first
llm_eval = ChatOpenAI(
...
# Initialize metric with LLM
faithfulness = Faithfulness(llm=llm_eval)
...
try:
    result = evaluate(
        ...
        # In newer Ragas, we might not need to pass llm/embeddings to evaluate() 
        # if they are already in the metrics, but it doesn't hurt.
    )
    print("Success!")
    print(result)
